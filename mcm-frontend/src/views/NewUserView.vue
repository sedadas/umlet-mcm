  ...
  <div class="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
    ...
    <div class="bg-white rounded-lg shadow-lg w-full max-w-4xl overflow-hidden">
      ...
      <form @submit.prevent="createNewUser">
        <div>
          <label>Email:</label>
          <input v-model="newUser.username" placeholder="Enter your email" type="email" required />
        </div>
        <div>
          <label>Password:</label>
          <input v-model="newUser.password" placeholder="Enter your password" type="password" required />
        </div>
        ...
        <div>
          <multiselect id="multiselect" v-model="newUser.roles" :options="userRoles" :multiple="true" :close-on-select="false" :clear-on-select="false"
                       :preserve-search="true" placeholder="Pick some" track-by="name" label="name" :preselect-first="true">
